...
import { calcCAGR, calcPercentChange, deriveTrendSignal, formatMarketCap, round1, round2, safeNumber, } from '../utils/formatting.js';
/**
...
function deriveHealthSignal(debtToEquity, currentRatio, interestCoverage) {
    ...
    if (counted === 0)
        return null;
    if (strongs >= 2 && weaks === 0)
        return 'STRONG';
    if (weaks >= 2)
        return 'WEAK';
    return 'ADEQUATE';
}
function deriveGrowthSignal(growthRates) {
    const clean = growthRates.filter((v) => v !== null && v !== undefined && !Number.isNaN(v));
    if (clean.length < 2)
        return null;
    const first = clean[0];
...
export async function getCompanyMetrics(symbol, period = 'annual', client) {
    ...
    const revenueGrowthYoy = revenue !== null && priorRevenue !== null && priorRevenue !== 0
    ...
    const totalCash = safeNumber(latestBs?.cashAndShortTermInvestments) ??
        safeNumber(latestBs?.cashAndCashEquivalents);
    const netDebt = safeNumber(latestBs?.netDebt) ??
        (totalDebt !== null && totalCash !== null ? totalDebt - totalCash : null);
    const debtToEquity = safeNumber(latestRt?.debtEquityRatio) ??
        safeNumber(latestKm?.debtToEquity);
    const currentRatio = safeNumber(latestRt?.currentRatio) ?? safeNumber(latestKm?.currentRatio);
    const interestCoverage = safeNumber(latestRt?.interestCoverage) ?? safeNumber(latestKm?.interestCoverage);
    ...
    const healthSignal = deriveHealthSignal(debtToEquityRounded, currentRatioRounded, interestCoverageRounded);
    // --- Cash Flow ---
    const operatingCashFlow = safeNumber(latestCf?.netCashProvidedByOperatingActivities) ??
    ...
    const fcfMargin = freeCashFlow !== null && revenue !== null && revenue > 0
    ...
    const revenueCagr = revenueSeries.length >= 2 && revenueYears > 0
        ? calcCAGR(revenueSeries[0], revenueSeries[revenueSeries.length - 1], revenueYears)
        : null;
    ...
    const sharesOut = safeNumber(latestInc?.weightedAverageShsOutDil) ??
        safeNumber(latestInc?.weightedAverageShsOut);
    const derivedDps = dividendsPaid !== null && sharesOut !== null && sharesOut > 0
...
//# sourceMappingURL=get-company-metrics.js.map